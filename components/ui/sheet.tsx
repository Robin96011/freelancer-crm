...
const sheetVariants = cva(
  "bg-background fixed z-50 gap-4 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      className={cn(
        sheetVariants({ side }),
        "flex w-[min(280px,88vw)] flex-col md:max-w-sm",
        className
      )}
      {...props}
    >
